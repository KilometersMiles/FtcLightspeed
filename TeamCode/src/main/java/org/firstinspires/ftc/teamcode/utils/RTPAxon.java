...
public class RTPAxon {
    ...
    private void initialize() {
        ...
        do {
            ...
            // Just get the angle, don't worry about it being > 1
            previousAngle = getCurrentAngle();
            ntry++;
            try {
                Thread.sleep(10);
            } catch (InterruptedException ignored) {}
        } while (ntry < 10); // Simplified check

        // --- THE FIX ---
        // OLD:
        // totalRotation = 0;
        // homeAngle = previousAngle; // Sets current pos as 0

        // NEW:
        totalRotation = previousAngle; // Starts at ACTUAL angle
        homeAngle = 0;                 // 0 is 0
        // ----------------
    ...
    // Get current angle from encoder (in radians)
    public double getCurrentAngleRadians() {
        if (servoEncoder == null) return 0;
        return (servoEncoder.getVoltage() / 3.3) * (direction.equals(Direction.REVERSE) ? (-2 * Math.PI) : (2 * Math.PI));
    }
    ...
    public synchronized void update() {
        ...
        double fTerm = kF * Math.signum(pTerm);
